...
class trajectory2seq(nn.Module):
    def __init__(self, hidden_dim, n_layers, int2symb, symb2int, dict_size, device, maxlen, with_attention=False, with_bidirectionnal=False):
        ...
        self.with_bidirectionnal = with_bidirectionnal

        # Definition des couches
        # Couches pour rnn
        self.embeding_decoder = nn.Embedding(self.dict_size, self.hidden_dim)
        self.encoder_layer = nn.GRU(2, self.hidden_dim, self.n_layers, batch_first=True, bidirectional=self.with_bidirectionnal)
        self.decoder_layer = nn.GRU(self.hidden_dim, self.hidden_dim, self.n_layers, batch_first=True, bidirectional=self.with_bidirectionnal)

        # Couches pour attention
        self.att_softmax = nn.Softmax()
        
        if self.with_bidirectionnal:
            self.att_combine = nn.Linear(self.hidden_dim*2*2, self.hidden_dim)
