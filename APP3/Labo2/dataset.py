...
class Fr_En(Dataset):
    ...
    def __init__(self, filename='fra.txt', n_samp=2000,start=0, samplelen=[7,10]):
        ...
        _dir_path = os.path.join(_dir_path, filename)
        with open(_dir_path, encoding='utf-8') as fp:
        ...
        for i in range(len(data['fr'])):
            if len(data['fr'][i]) > self.max_len['fr']:
                self.max_len['fr'] = len(data['fr'][i])
        self.max_len['fr'] += 1

        self.max_len['en'] = 0
        for i in range(len(data['en'])):
            if len(data['en'][i]) > self.max_len['en']:
                self.max_len['en'] = len(data['en'][i])
        self.max_len['en'] += 1

        for i in range(len(data['fr'])):
            data['fr'][i] += [self.stop_symbol] + [self.pad_symbol] * (self.max_len['fr'] - len(data['fr'][i]) - 1)

        for i in range(len(data['en'])):
            data['en'][i] += [self.stop_symbol] + [self.pad_symbol] * (self.max_len['en'] - len(data['en'][i]) - 1)
